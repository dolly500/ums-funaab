...
exports.getProfile = async (req, res, next) => {
  ...
  const programData = await queryParamPromise(sql3, [profileData[0].program_id]);
...
exports.getResults = async (req, res, next) => {
  ...
  const profileData = await queryParamPromise(sql, [req.user]);
  const deptName = await queryParamPromise(sql2, [req.user]);
  const programData = await queryParamPromise(sql3, [req.user]);
...
exports.printResults = async (req, res, next) => {
  const semester = req.params.semester;

  const sql = 
  'SELECT marks.cat_marks, marks.exam_marks, marks.c_id, marks.semester, course.name, course.credits FROM marks INNER JOIN course ON marks.c_id=course.c_id WHERE s_id = ? AND marks.semester = ?';

  const result = await queryParamPromise(sql, [req.user, semester]);

  if (result.length < 1){
    req.flash('error_msg', 'Selected Semester not completed!');
    return res.redirect('/student/results');
  }

  // console.log(result[0]);
  // console.log(programData)
  return res.render('Student/printResult', {
    data: result,
    page_name: 'result',
  });
};
